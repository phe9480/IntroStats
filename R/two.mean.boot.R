#' Bootstrapping method comparing two population means
#'
#' This function performs the bootstrapping method to compare two population means
#' 
#' @param x1 sample 1
#' @param x2 sample 2
#' @param conflev Confidence level
#' @param tail type of test "two", "left", "right". Default "two"
#' @param n.boot number of bootstrapping samples, default 10000
...
#' two.mean.boot(x1, x2, tail="two", n.boot=10000, seed=2023)
#' 
#' 
#' @export 
#'
two.mean.boot <- function(x1, x2, tail="two", conflev=0.95,
                          n.boot=10000, seed=2023){
  ...
  #Bootstrap sampling x variable and 
  #Determine the empirical distribution of the test statistic T
  set.seed(seed)
  T <- rep(NA, n.boot)
  
  for (i in 1:n.boot){
    xi <- sample(x, replace=TRUE) #Sampling with replacement
    x1i <- xi[group == 1]   #New group assignment
    x2i <- xi[group == 2]
